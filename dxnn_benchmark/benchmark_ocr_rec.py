#!/usr/bin/env python3
"""
OCR Recognition 모델의 순수 DX Engine 추론 성능 측정 스크립트

전체 파이프라인 없이 OCR Recognition 모델의 순수 추론 속도만 측정합니다.
"""
import time
import numpy as np
...
def benchmark_ocr_rec_dxnn():
    """OCR Recognition 모델의 DX Engine 추론 성능 측정"""
    
    # =========================================================================
    # 모델 경로 설정
    # =========================================================================
    project_root = Path(__file__).parent.parent.absolute()
    ocr_rec_model_path = project_root / "dxnn_models" / "ch_PP-OCRv5_rec_server_infer.dxnn"
    
    if not ocr_rec_model_path.exists():
        logger.error(f"OCR Recognition 모델을 찾을 수 없습니다: {ocr_rec_model_path}")
        return
    
    logger.info("=" * 80)
    logger.info("OCR Recognition 모델 순수 DXNN 추론 성능 측정")
    logger.info("=" * 80)
    logger.info(f"모델 경로: {ocr_rec_model_path}")
    ...
    session = InferenceEngine(str(ocr_rec_model_path))
    session_time = time.perf_counter() - session_start
    logger.info(f"✓ 세션 생성 완료: {session_time:.3f}초")
    
    # 입력/출력 정보 확인
    input_name = 'x'
    input_shape = [1, 3, 48, 640]  # OCR Rec 표준 입력: 높이 48, 너비 가변 (여기서는 640)
    ...
    for i, name in enumerate(output_names):
        output_shape = [1, 'seq_len', 'num_classes']  # 동적 크기
        logger.info(f"  출력 {i}: {name} - shape: {output_shape}")
    
    # =========================================================================
    # 테스트 이미지 생성 (고정 크기: 48x640 - OCR Rec 모델의 표준 입력)
    # =========================================================================
    height, width = 48, 640
