...
class DXNNRecognitionInference:
    ...
    def preprocess(self, img: np.ndarray, max_wh_ratio: Optional[float] = None) -> np.ndarray:
        """
        단일 이미지 전처리 (DXNN용 - Channel-last format, uint8)
        
        전처리 과정 (DXNN 모델용):
        1. Resize: aspect ratio 유지하며 높이 48로 조정
        2. Padding: 오른쪽에 0으로 패딩 (고정 너비 640)
        3. 출력: (H, W, C) 형식, uint8 [0, 255] - normalization 하지 않음
        
        주의: DXNN 모델은 uint8 입력을 받으며, 내부에서 normalization 수행
        
        Args:
            img: 입력 이미지 (H, W, C), BGR 포맷, uint8
            max_wh_ratio: 최대 가로/세로 비율 (사용하지 않음, 호환성 유지용)
            
        Returns:
            전처리된 이미지 (H, W, C), uint8, [0, 255]
        """
        imgC, imgH, imgW_base = self.rec_image_shape
        
        # 최대 너비 고정 (640)
        imgW = imgW_base  # 640으로 고정
        ...
        # 2. 패딩 (오른쪽에 0으로 패딩) - Channel-last format (H, W, C)
        # DXNN 모델은 uint8 입력을 받으므로 normalization 하지 않음
    ...
    def preprocess_batch(self, img_list: List[np.ndarray]) -> np.ndarray:
        """
        배치 이미지 전처리 (고정 너비 640, Channel-last format)
        
        Args:
            img_list: 입력 이미지 리스트 (각각 H, W, C)
    ...
    def inference(self, img_input: np.ndarray) -> np.ndarray:
        ...
        Args:
            img_input: 전처리된 이미지 (B, H, W, C) 또는 (H, W, C)
        ...
        outputs = self.session.run([img_input])
...
def main():
    ...
    # 인식기 초기화
    recognizer = DXNNRecognitionInference(
