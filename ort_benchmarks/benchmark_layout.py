...
def benchmark_layout_ort():
    """Layout 모델의 ONNX Runtime 추론 성능 측정"""
    
    # =========================================================================
    # 모델 경로 설정
    # =========================================================================
    project_root = Path(__file__).parent.parent.absolute()
    layout_model_path = project_root / "onnx_models" / "pp_doclayout_l.onnx"
    ...
    logger.info("=" * 80)
    logger.info("Layout 모델 순수 ONNX Runtime 추론 성능 측정")
    logger.info("=" * 80)
    logger.info(f"모델 경로: {layout_model_path}")
    ...
    # 입력/출력 정보 확인
    logger.info(f"입력 개수: {len(session.get_inputs())}")
    for i, inp in enumerate(session.get_inputs()):
        logger.info(f"  입력 {i}: {inp.name} - shape: {inp.shape}")
    ...
    for _ in range(3):
        _ = session.run(output_names, input_feed)
